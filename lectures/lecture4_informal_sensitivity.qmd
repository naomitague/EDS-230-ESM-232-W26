...
First, come put with multiple samples from our function

Second, run model over generated sample
...
site2 <- eff %>% map(~ solarpv(area = 0.1,  # Give it all the paramters 
  solar = sierraczosolar, clr = "green", ethresh = ethresh,
...
# convert from tibble to a data frame with nicer labels 
...
### Controlling for a 15% uncertainty

```{r}
# Creating +- 15% Uncertainity 
ethresh <- rnorm(mean = 10000, sd = 0.15*10000, n = 20)
# Other way to do this 
base_thresh <- 10000
deviation <- 0.15
ethresh_2 <- runif(
  base_thresh + deviation * base_thresh, 
  base_thresh - deviation * base_thresh, n = 20)

# Run through function 
site2 <- ethresh %>% 
  map(~ solarpv(area = 0.1,  # Give it all the paramters 
...
ggplot(site2df, aes(year, elect, group = year)) +
  geom_boxplot() +
  labs(y = "Electricity in W") +
  geom_line(data = site2_average, aes(year, elect, group = 1), col = "orange", lwd= 1)
