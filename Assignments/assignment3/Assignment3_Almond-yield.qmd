...
source(here::here("Assignments", "assignment3", "almond_function.R"))
source(here::here("Assignments", "assignment3", "winegrape_function.R"))

```


# Monthly Averages 
Monthly Averages (per year) *aggregated to state scale / crop area* (idk how to scape to state size)
...
$$
Y = -0.015T_{nFeb} - 0.0046T_{nFeb}^2 - 0.07P_{Jan} + 0.0043P_{Jan}^2 + 0.28
$$
```{r}
# Use function 
almond_anomaly <- almond_function(almond_df = almond_subset, 
                                   graph = TRUE)
almond_anomaly
...
**Wine Grapes Equation**

$$
Y = 2.65T_{n4} - 0.17T_{n4}^2 + 4.78P_{6} - 4.93P_{6}^2 - 2.24P_{-9} + 1.54P_{-9}^2 - 10.50
$$

*I do not understand the -9 meaning. Just need to make the selection of `precip_sept_pre` correct then the function will be good!*

```{r}
# Use function 
winegrape_anomaly <- winegrape_function(almond_df = almond_subset, 
                                   graph = TRUE)
winegrape_anomaly

```

**Table Grapes Equation**

$$
Y = 6.93T_{n7} - 0.19T_{n7}^2 + 2.61T{n4} - 0.15T_{n4}^2 + 0.035P_{1} + 0.024P_{1}^2 + 1.171P_{-10}^2 - 0.673P_{-10}^2 - 73.89
$$
